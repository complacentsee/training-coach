...
func (g *grader) tools(m *activityMetrics, posted *bool, result **gradeResult) []llmTool {
	...
	return []llmTool{
		...
		{
			Name:        "post_grade",
			Description: "Post the day's grade entry. Callable exactly once; val is the grade letter, or DNF where something outside the training stopped the session; note is the one-paragraph reasoning.",
...
2. Decide the grade:
   ...
   - DNF is a verdict, not a letter, and it is the right one when the session was stopped by something outside the training: a mechanical failure, an injury or illness that started during it, weather that made it unsafe, an interruption the athlete did not choose. Post val "DNF" and say in the note what stopped it and what was delivered before it did. It is NOT for a session abandoned through fatigue or choice — that takes its letter, usually a low one, because doing the prescribed work is what puts an athlete over the cap and a verdict that excused quitting would reward it. The athlete's own entries are where the reason lives: an unfinished session with no stated cause is not a DNF.
